...
% Diretrizes da banca (Prof. Tanara): fonte sem serifa, corpo grande,
% tabelas/legendas legiveis, imagens apenas funcionais.
% ============================================================================
...
% --- Fonte sem serifa (diretriz Prof. Tanara) ------------------------------
\usepackage{helvet}
...
\begin{frame}{Três sinais de alerta}
  \begin{enumerate}
    \item \textbf{Matriz de confusão assimétrica:} \textit{precision}(Desce) = 1{,}00 com \textit{recall}(Desce) = 0{,}20 --- o modelo acertou apenas \alert{11 das 54 quedas reais}
    \item \textbf{Acurácia suspeita:} 76{,}3\% vs.\ \alert{69{,}5\%} da classe majoritária na janela de teste (\textit{base rate} de treino 59/41)
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item Nesta janela o IC é \alert{largo demais} para separar 0{,}709 do \textit{baseline}; a separação só emerge no CV
    \item Controle de dimensionalidade (20 subconjuntos aleatórios de 5 dims Ollama): AUC $\approx 0{,}509$
  \end{itemize}
\end{frame}
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Protocolo reforçado: \textbf{880 execuções} (\textit{folds} expandidos $\times$ 10 sementes $\times$ 2 modelos)
