...
% Apresentação ~20 min | Beamer | pt-BR
...
\begin{frame}{Roteiro}
  \begin{columns}[T]
    ...
    \begin{enumerate}
      ...
      \item Investigação metodológica ($\sim$1.510 execuções)
...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \end{tabular}
    \end{table}
    \footnotesize REST API do WordPress, retentativas com \textit{backoff}, extração paralela.
    \column{0.48\textwidth}
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59\% ``Sobe'' / 41\% ``Desce''
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \alert{Risco conhecido:} \textit{forward-fill} de sentimento em dias sem notícia (discutido na análise)
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  \begin{columns}[T]
    ...
    \column{0.46\textwidth}
    \begin{itemize}
      \item Transformer + FinBERT: \alert{ROC-AUC = 0{,}709}, acurácia 76{,}3\%
...
\begin{frame}{Três sinais de alerta}
  \begin{enumerate}
    \item \textbf{Matriz de confusão assimétrica:} \textit{precision}(Desce) = 1{,}00 com \textit{recall}(Desce) = 0{,}15 --- o modelo previu ``Desce'' apenas \alert{11 vezes em 177 amostras}
    \item \textbf{Acurácia suspeita:} 76{,}3\% próxima da classe majoritária ($\sim$69\% ``Sobe'')
...
% ---------------------------------------------------------------- 11. Investigação
\begin{frame}{Investigação metodológica: 13 experimentos, $\sim$1.510 execuções}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{table}
      \scriptsize
      \begin{tabular}{lr}
        ...
        \midrule
        Baselines (autoregressivo + ingênuos) & 7 \\
        Controle de dimensionalidade          & 20 \\
        Multi-seed (ITUB4 + multi-ticker)     & 160 \\
        Ensemble + backtest                   & 20 \\
        Expanding-window CV                   & 145 \\
        VALE3 \textit{deep-dive}              & 880 \\
        Ablation PRICE/SENT/PRICE+SENT        & 225 \\
        Validação TCN + varredura horizontes  & 51 \\
        \midrule
    ...
    \textbf{Regras fixas em todos os experimentos}
    \begin{itemize}
      \item Hiperparâmetros congelados (sem otimização por \textit{fold})
      \item \texttt{StandardScaler} ajustado só no treino de cada \textit{fold}
      \item Bootstrap CI com 1.000 reamostras
      \item Teste de Wilcoxon para comparações pareadas
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  \begin{table}
    ...
    \begin{tabular}{llc}
      ...
      \textbf{Autoregressivo (XGB)} & \textbf{5 \textit{features} de preço} & \textbf{0{,}658 [0{,}565; 0{,}744]} \\
      \bottomrule
    \end{tabular}
  \end{table}
  \vspace{1ex}
  \begin{itemize}
    ...
    \item O IC do \textit{baseline} \alert{já engloba o 0{,}709} do Transformer + FinBERT
    \item Controle de dimensionalidade (20 subconjuntos aleatórios de 5 dims Ollama): AUC $0{,}509 \pm 0{,}057$
  \end{itemize}
\end{frame}
...
\begin{frame}{Multi-seed: o colapso bimodal}
  \begin{columns}[T]
    ...
    \includegraphics[width=\textwidth]{multi_seed_histograms.png}
    \column{0.38\textwidth}
    \begin{itemize}
      \item Recompilação com semente 42: AUC despenca de 0{,}709 para \alert{0{,}442}
      \item 20 sementes (ITUB4): Transformer com std = \alert{0{,}261} ($21\times$ o \textit{baseline}), min 0{,}080, max 0{,}931
      \item Distribuição \textbf{bimodal}: o modelo colapsa para ``sempre Sobe'' ou ``sempre Desce''
      \item \textit{Baseline}: estável, std = 0{,}012
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \includegraphics[width=\textwidth]{expanding_cv_overtime.png}
    \column{0.42\textwidth}
    \begin{table}
      \footnotesize
      {\scriptsize\itshape AUC médio (5 \textit{folds} $\times$ 5 sementes)}\\[0.5ex]
      \begin{tabular}{lccc}
        \toprule
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    \column{0.58\textwidth}
    \includegraphics[width=\textwidth]{vale3_deepdive_hist.png}
    \column{0.38\textwidth}
    \begin{itemize}
      \item Único ativo com $\Delta > 0$ ($+$0{,}036) na análise principal
      \item Protocolo reforçado: \textbf{52 \textit{folds} $\times$ 10 sementes}
      \item Resultado: \alert{não significativo} (Wilcoxon $p = 0{,}194$)
      \item Transformer: bimodal severo --- picos em AUC $<0{,}05$ e $>0{,}95$, assinatura de classificador degenerado
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \includegraphics[width=\textwidth]{ablation_boxplot.png}
    \column{0.45\textwidth}
    \begin{table}
      \scriptsize
      {\scriptsize\itshape XGBoost, 225 treinamentos, $h=21$}\\[0.5ex]
      \begin{tabular}{lccc}
        \toprule
        \textbf{Ativo} & \textbf{PRICE} & \textbf{SENT} & $\boldsymbol{\Delta}$\textsubscript{P+S$-$P} \\
        \midrule
        ITUB4 & 0{,}684 & 0{,}436 & $-$0{,}033 \\
        PETR4 & 0{,}692 & 0{,}494 & $-$0{,}016 \\
        VALE3 & 0{,}609 & 0{,}510 & $+$0{,}058 \\
        \midrule
        \textbf{Média} & \textbf{0{,}662} & 0{,}480 & \alert{\textbf{$+$0{,}003}} \\
        \bottomrule
      \end{tabular}
    \end{table}
    \begin{itemize}
      \item $\Delta = +0{,}003$; Wilcoxon $p = 0{,}49$; IC 95\% $[-0{,}012;\ +0{,}018]$
      \item SENT sozinho: \alert{abaixo do acaso}
      \item Efeito 44--74$\times$ menor que o mínimo detectável (MDE 0{,}13--0{,}22)
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Contribuições e protocolos mínimos propostos}
  \begin{columns}[T]
    ...
    \begin{enumerate}
      ...
      \item Primeira demonstração quantificada do viés de janela única em dados brasileiros ($>$1.500 execuções)
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill} --- mas joga \emph{contra} a conclusão nula
