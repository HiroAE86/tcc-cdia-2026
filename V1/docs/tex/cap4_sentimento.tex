...
Para cada artigo, o FinBERT-PT-BR produz três \textit{logits} --- pontuações brutas não normalizadas, convertidas em probabilidades via \textit{softmax} (ver Glossário e Apêndice~\ref{ap:artigos}) --- correspondentes às classes POSITIVO, NEGATIVO e NEUTRO. Diferentemente de modelos de sentimento genéricos, foi calibrado para vocabulário financeiro específico --- termos como ``alavancagem'', ``\textit{guidance}'' ou ``\textit{rating}'' carregam polaridade que um léxico de propósito geral não captura.
