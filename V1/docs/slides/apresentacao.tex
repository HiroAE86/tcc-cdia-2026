...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- compensado por \alert{peso na perda, não por reamostragem}
    \end{itemize}
    {\footnotesize Reamostrar 50/50 quebraria a ordem temporal e a \textit{base rate} real; o peso na perda dá o mesmo gradiente. E o colapso \alert{persiste mesmo com o peso} $\Rightarrow$ desbalanceamento não era a causa raiz.}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill} propaga só o passado do sentimento; preço e alvo vêm de OHLCV sem \textit{ff}
    \item \alert{Resíduo intradiário não auditado} --- mas \textit{ff} só do sentimento: mesmo que vaze a favor, o nulo $+$0{,}003 vira teto otimista. \alert{Leakage não fabrica efeito que não existe} (ver slide 19)
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. \alert{O XGBoost 0{,}670 também é artefato: cai a 0{,}665 \textit{multi-seed} (s.16) --- a regra ``janela única é artefato'' vale para ele igual ao Transformer.}}
\end{frame}
...
% ---------------------------------------------------------------- 11. Investigação
\begin{frame}{8 experimentos pré-registrados (1.435 execuções)}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{table}
      \footnotesize
    ...
    \end{itemize}
    \begin{block}{8 perguntas = 8 p-valores}
      {\footnotesize 6 células ativo$\times$horizonte (3$\times$2) no \textit{ablation}; 1 com ganho (VALE3) $=$ o esperado por acaso. Corrigir múltiplas comparações só \alert{reforça o NULO} do sentimento; o $|\Delta|=0{,}255$ de preço passa Bonferroni com folga.}
    \end{block}
  \end{columns}
  \vspace{1ex}
  {\footnotesize 1.435 pré-registrados $+$ $\sim$80 \textit{sanity checks} $\approx$ 1.515 total; só os 8 pré-registrados têm p-valor --- os \textit{sanity checks} NÃO geram teste de hipótese.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno:} o \textit{backtest} \textit{ensemble} (slide 11) \alert{não evidenciou ganho após custos} e foi descontinuado $\Rightarrow$ compatível com eficiência \alert{fraca}, não semi-forte
  \end{itemize}
  \vspace{0.5ex}
  {\footnotesize Captura momentum/autocorrelação de curto prazo (Lo, 2004); $h=21$ com janelas sobrepostas infla a autocorrelação do alvo. \alert{0{,}658 $=$ janela única (IC largo); 0{,}667 $=$ média 5 \textit{folds} CV (slide 14) --- protocolos distintos, níveis distintos. O invariante: o preço vence o sentimento nos dois.}}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lccc}
        ...
        VALE3 & 0{,}599 & 0{,}635 & $+$0{,}036\,\alert{(n.s.; $p=0{,}194$ no \textit{deep-dive} 880 \textit{runs})} \\
        \midrule
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Sinal que troca de direção entre experimentos ($+$0{,}036, $+$0{,}058, aqui negativo) é a própria evidência de que \alert{não há sinal real} --- nenhum sobrevive ao teste
    \end{itemize}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{(n.s.; significância testada no \textit{deep-dive} de 880 \textit{runs}, $p=0{,}194$ --- s.15)} \\
        \midrule
    ...
    \begin{center}\Large\alert{$\Delta = +0{,}003$}\ \ ($p = 0{,}49$, \textit{ablation} 225)\\[0.3ex]
    {\footnotesize \alert{O mesmo desenho detecta preço (0{,}667 estável) $\Rightarrow$ protocolo é sensível, não cego.} SENT abaixo do acaso.}\end{center}
    {\scriptsize MDE/IC e \textit{forward-fill} na reserva: MDE 0{,}13--0{,}22 é \textit{por ativo} ($n$ pequeno); o IC do $\Delta$ MÉDIO agregado [$-$0{,}012; $+$0{,}018] é estreito por agregação --- não há contradição. O nulo TEM IC; o 0{,}709 não tinha.}
  \end{columns}
\end{frame}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill} (só sentimento; intradiário, não auditado) --- conclusão nula é robusta à direção do viés: se favorece o sentimento, $\Delta=+0{,}003$ é teto otimista; se for contra, o efeito real é ainda menor. O argumento garante que o \textit{ff} \alert{não FABRICA um efeito positivo inexistente} --- não precisa distinguir zero de negativo, pois ambos sustentam a conclusão
