...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. \alert{O XGBoost 0{,}670 também é artefato de janela única} (0{,}670 = s.9, janela única; 0{,}665 = s.16, \textit{multi-seed} --- reconciliação em Q\&A).}
\end{frame}
...
\begin{frame}{8 experimentos com pergunta e métrica definidas a priori (1.435 execuções)}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Único claim positivo (preço vence, $|\Delta|\approx$0{,}255 $\approx$ 6$\times$ o desvio do \textit{baseline}) sobrevive a Bonferroni das 6 células; a correção só \alert{reforça} o nulo do sentimento
    \end{itemize}
  \end{columns}
  \vspace{1ex}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade direcional, não retorno:} a autocorrelação de curto prazo é conhecida (Lo, 2004 --- mercados adaptativos) e $h=21$ com janelas sobrepostas a infla; o \textit{backtest} \textit{ensemble} foi descontinuado \alert{sem evidência de lucro após custos} --- não reivindico lucratividade
  \end{itemize}
  \vspace{0.5ex}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    {\scriptsize Agregado: \alert{potência suficiente} (IC exclui efeito útil) $\Rightarrow$ nulo conclusivo. Por ativo: subpotenciado (por isso agrego). MDE e IC do $\Delta$ na reserva.}
  \end{columns}
\end{frame}
