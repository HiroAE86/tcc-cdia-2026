...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- compensado por \alert{peso na perda}, \alert{não por reamostragem}
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções nos 8 experimentos pré-registrados; contrastes formais por Wilcoxon pareado / bootstrap CI conforme o desenho. As $3\times2=6$ células ativo$\times$horizonte do \textit{ablation} são o único espaço de busca de efeito positivo --- 1 deu ganho, o esperado por acaso. Os demais até $\sim$1.500 são \textit{sanity checks} sem teste de hipótese.}
\end{frame}
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Reforço: \textbf{880 execuções} (\textit{folds} expandidos $\times$ 10 sementes, 2 modelos)
