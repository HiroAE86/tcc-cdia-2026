...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- compensado por \alert{peso de classe na perda/amostra}, \alert{não por reamostragem}
    \end{itemize}
  \end{columns}
  \vspace{1ex}
  {\footnotesize Reamostragem 50/50 quebraria a ordem temporal (\textit{walk-forward}) e a \textit{base rate} real do mercado; compensado por peso (XGBoost \texttt{scale\_pos\_weight}; neurais \texttt{pos\_weight}$\approx$1{,}38). O colapso bimodal persiste MESMO com a compensação $\Rightarrow$ desbalanceamento não era a causa raiz.}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \alert{Resíduo intradiário não auditado sistematicamente --- limitação declarada (slide 19)}
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, janela única, 1 semente, sem IC --- refutado como artefato nos slides 13--16. O 0{,}670 do XGBoost é janela única (sentimento bruto vs \textit{embeddings}); na avaliação rigorosa (ablation multi-seed, slide 16) o mesmo modelo dá 0{,}665 --- a regra de janela única vale também para o XGBoost.}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções pré-registradas = 8 experimentos, cada um com 1 p-valor; os $>$65 \textit{sanity checks} restantes (até $>$1.500) não geram teste de hipótese. \alert{Só os 8 pré-registrados têm p-valor; correção por múltiplas comparações só REFORÇA o nulo do sentimento.}}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \end{itemize}
  \vspace{0.5ex}
  {\footnotesize AUC mede separabilidade, não lucro; captura momentum/autocorrelação de curto prazo (Lo, 2004), que não virou retorno após custos no \textit{backtest} --- compatível com eficiência semi-forte.}
\end{frame}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \begin{block}{Invariante}
      {\footnotesize Na VALE3 o ganho \alert{NUNCA é significativo} --- \textit{deep-dive} 880 \textit{runs}, $p=0{,}194$. O $+$0{,}058 abaixo é experimento distinto.}
    \end{block}
    \begin{table}
      \footnotesize
    ...
    \begin{itemize}
      \item O mesmo desenho detecta o sinal de preço (0{,}667, estável) --- protocolo é sensível quando há efeito; sentimento está 44--74$\times$ abaixo do MDE
    \end{itemize}
  \end{columns}
\end{frame}
