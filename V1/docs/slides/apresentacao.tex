...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \alert{Mesmo o resíduo intradiário não auditado joga A FAVOR do sentimento $\Rightarrow$ $+$0{,}003 já é limite superior otimista}
  \end{itemize}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções pré-registradas (8 experimentos); os $>$65 \textit{sanity checks} não geram p-valor. \alert{Correção por múltiplas comparações só REFORÇA o nulo do sentimento; e o $|\Delta|=0{,}255$ (preço) passa Bonferroni das 6 células com folga ($\sim$6$\times$ o desvio do \textit{baseline}).}}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item A estimativa de janela única é tão incerta (IC largo) que \alert{não distingue} 0{,}709 do \textit{baseline} --- a separação do acaso só emerge no CV/multi-seed (slide 14), onde a direção inverte. O 0{,}658 (1 janela) e o 0{,}667 (média CV) são a MESMA grandeza sob protocolos distintos
  \end{itemize}
  \vspace{0.5ex}
  {\footnotesize AUC mede separabilidade, não lucro; ``\textit{baseline} 0{,}70 refuta a HME?'' --- reserva de Q\&A.}
\end{frame}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \column{0.42\textwidth}
    {\footnotesize\alert{VALE3: ganho do sentimento nunca é significativo ($p=0{,}194$, 880 \textit{runs}); $\Delta$ abaixo vêm de experimentos distintos.}}\\[0.5ex]
    \begin{table}
      \footnotesize
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    {\footnotesize\alert{VALE3: ganho do sentimento nunca é significativo ($p=0{,}194$, 880 \textit{runs}); o $+$0{,}058 abaixo é experimento distinto.}}\\[0.5ex]
    \begin{table}
      \footnotesize
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill} (só sentimento; intradiário, não auditado) --- viés inflaria o sentimento, logo $\Delta=+0{,}003$ é teto otimista
