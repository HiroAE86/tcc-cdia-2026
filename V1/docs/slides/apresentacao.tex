...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)}; a janela de teste calhou 69{,}5/30{,}5 --- ver slide 10
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill} de sentimento em dias sem notícia \alert{propaga o passado (não vaza futuro de mercado)}; o risco residual é intradiário e \alert{viesa a favor} do sentimento $\Rightarrow$ nosso nulo é um limite superior otimista
  \end{itemize}
\end{frame}
...
\begin{frame}{Três sinais de alerta}
  ...
  \end{enumerate}
  \vspace{1ex}
  {\footnotesize Desbalanceamento compensado por \alert{peso na perda} (\texttt{pos\_weight}$\approx$1{,}38 / \texttt{scale\_pos\_weight}), não por reamostragem --- preserva a ordem temporal e a \textit{base rate} real. O colapso persiste assim mesmo $\Rightarrow$ o desbalanceamento não é a causa raiz.}
  \vspace{1ex}
  \begin{block}{Decisão}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize 1.435 nos 8 experimentos centrais; $>$1.500 contando os diagnósticos exploratórios.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item {\footnotesize AUC mede \emph{separabilidade}, não retorno: o \textit{ensemble}+\textit{backtest} não gerou ganho após custos (descontinuado).}
  \end{itemize}
\end{frame}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3 $+$0{,}058 aqui, mas o \textit{deep-dive} de 880 \textit{runs} (slide 15) mata: $p=0{,}194$.} São 3 medidas distintas (modelos/contrastes/$n$ diferentes) $\Rightarrow$ magnitude instável = a prova de que não é sinal.}
  \end{columns}
\end{frame}
