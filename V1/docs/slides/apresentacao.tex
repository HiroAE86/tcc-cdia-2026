...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{ff} só do passado do sentimento (preço e alvo vêm de OHLCV sem \textit{ff}); resíduo intradiário não auditado --- robustez completa no slide 19
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. \alert{O XGBoost 0{,}670 também é artefato de janela única} (reconciliação 0{,}670 vs 0{,}665 em Q\&A).}
\end{frame}
...
\begin{frame}{8 experimentos com pergunta e métrica definidas a priori (1.435 execuções)}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Claim científico central $=$ o \alert{nulo do sentimento}: múltiplas comparações só o \alert{reforçam}. ``Preço vence'' é secundário e robusto por \textbf{magnitude} ($|\Delta|\approx$0{,}255 $\approx$ 6$\times$ o desvio do \textit{baseline}) --- sobrevive a correção bem além de $/6$
    \end{itemize}
  \end{columns}
  \vspace{1ex}
  {\footnotesize 1.435 execuções nos 8 experimentos centrais $+$ $\sim$80 \textit{sanity checks} $\approx$ 1.515 total --- só os 8 centrais têm p-valor.}
\end{frame}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \begin{block}{Invariante}
      {\footnotesize \alert{VALE3: ganho do sentimento nunca significativo ($p=0{,}194$, 880 \textit{runs}); 3 experimentos distintos $=$ magnitudes distintas.}}
    \end{block}
    \begin{table}
      \footnotesize
      ...
      \begin{tabular}{lccc}
        ...
        VALE3 & 0{,}599 & 0{,}635 & $+$0{,}036\,\alert{(Transf.\ vs preço, 5$\times$5; n.s.; $p=0{,}194$)} \\
        \midrule
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \column{0.38\textwidth}
    {\footnotesize\alert{VALE3: ganho do sentimento nunca significativo ($p=0{,}194$, 880 \textit{runs}); 3 experimentos distintos $=$ magnitudes distintas.}}\\[0.5ex]
    \begin{itemize}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{(célula isolada n.s.; reforço \textit{deep-dive} $p=0{,}194$, s.15)} \\
        \midrule
        \textbf{Média} & \textbf{0{,}662} & 0{,}480 & 0{,}665 & \alert{\textbf{$+$0{,}003}} \\
        \bottomrule
      \end{tabular}}
    \end{table}
    {\scriptsize Dois de três ativos \alert{pioram} com sentimento (ITUB4 $-$0{,}033; PETR4 $-$0{,}016); a média $+$0{,}003 não é diluição de um efeito --- é ausência dele.}
    \begin{center}\Large\alert{$\Delta = +0{,}003$ \ ($p = 0{,}49$)} --- estatisticamente zero\end{center}
    {\scriptsize Por \textbf{ativo} subpotenciado (MDE 0{,}13--0{,}22); por isso \textbf{agrego} os 225 \textit{runs} $\Rightarrow$ IC do $\Delta$ médio [$-$0{,}012; $+$0{,}018], estreito, exclui efeito útil. O mesmo desenho \alert{detecta preço} (0{,}667). Logo: nulo conclusivo no agregado, não por ativo.}
  \end{columns}
\end{frame}
...
\begin{frame}{Contribuições e protocolos mínimos propostos}
  \begin{columns}[T]
    ...
    \begin{enumerate}
      \item Pipeline replicável e versionado (o README preserva a narrativa inicial \alert{de propósito}, como registro do viés de confirmação --- errata no topo aponta para o cap. 5)
