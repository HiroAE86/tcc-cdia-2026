...
\begin{frame}{Dados: notícias + mercado}
  ...
  \end{columns}
  \vspace{0.5ex}
  {\footnotesize Não reamostrei 50/50: quebraria a ordem temporal do \textit{walk-forward} e a \textit{base rate} real (59/41); compensei por peso de classe (detalhe no Q\&A). Decisivo: o colapso persiste no XGBoost $\Rightarrow$ desbalanceamento não é a causa raiz.}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textbf{Risco conhecido:} \textit{forward-fill} de sentimento propaga só o \alert{passado}; mesmo no pior caso (viés a favor do sentimento) o nulo $\Delta=+0{,}003$ seria limite \alert{superior} otimista (risco intradiário no slide 19)
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. \alert{Preço $\sim$0{,}66 é estável; nenhum número é sinal do sentimento:} 0{,}670 (XGB, janela única, sent.\ bruto) $\to$ 0{,}665 (XGB, multi-seed, preço+sent.) $\to$ 0{,}662 (XGB, PRICE-only, \textit{ablation}). Protocolos distintos, todos $\sim$0{,}66.}
\end{frame}
...
\begin{frame}{Bateria de investigação: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Só os 8 experimentos centrais geram teste de hipótese; os $\sim$80 extras são \textit{sanity checks} sem p-valor. Logo: 1.435 pré-registradas (pergunta e métrica a priori) $+$ $\sim$80 \textit{sanity checks} $\approx$ 1.515 --- o número maior não é \textit{padding}.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno} (Lo, 2004; $h=21$ sobreposto infla a autocorrelação). \textbf{O 0{,}667 NÃO é lucrável:} é momentum/autocorrelação, e o \textit{backtest ensemble} não superou custos/\textit{slippage} (s.19) --- não gera alfa
  \end{itemize}
  \vspace{0.5ex}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \end{itemize}
    {\scriptsize VALE3: $\Delta$ varia entre experimentos ($n$ distinto); invariante $=$ nunca significativo, \textit{deep-dive} $p=0{,}194$ (s.15).}
  \end{columns}
\end{frame}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \end{tabular}}
    \end{table}
    {\scriptsize Dois de três ativos \alert{pioram} com sentimento (ITUB4 $-$0{,}033; PETR4 $-$0{,}016). Níveis de preço diferem entre s.14 (CV, Transf.\ vs preço) e s.16 (\textit{ablation}, XGB) por protocolo/$n$ distintos; o invariante é o $p$, não o nível.}
    \begin{center}\large\alert{$\Delta = +0{,}003$ \ ($p = 0{,}49$); IC95\% $[-0{,}012;\,+0{,}018]$}\end{center}
    {\scriptsize Teto $\sim$3$\times$ abaixo do efeito útil; não é só falta de poder --- o mesmo desenho detecta preço (0{,}667). (MDE por ativo e demais reconciliações no Q\&A.)}
  \end{columns}
\end{frame}
