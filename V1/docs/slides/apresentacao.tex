...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- compensado por \alert{peso de classe, não por reamostragem}
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \alert{Resíduo intradiário não auditado --- ver slide 19}
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. (XGBoost 0{,}670 é \textit{janela única}; cai a 0{,}665 \textit{multi-seed} na ablation, s.16 --- a regra ``janela única é artefato'' vale para o XGBoost também.)}
\end{frame}
...
\begin{frame}{8 experimentos pré-registrados (1.435 execuções)}
  \begin{columns}[T]
    ...
    \begin{block}{8 perguntas = 8 p-valores}
      {\footnotesize Correção de múltiplas comparações só \alert{reforça o NULO} (Bonferroni torna mais difícil achar positivo).}
    \end{block}
  \end{columns}
  \vspace{1ex}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno:} o \textit{backtest} \textit{ensemble} (slide 11) não virou lucro após custos $\Rightarrow$ compatível com eficiência \alert{fraca}, não semi-forte
  \end{itemize}
  \vspace{0.5ex}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \begin{block}{Invariante}
      {\footnotesize Na VALE3 o ganho \alert{NUNCA é significativo} --- \textit{deep-dive} 880 \textit{runs}, $p=0{,}194$. Os $\Delta$ abaixo vêm de experimentos distintos.}
    \end{block}
    \begin{table}
      \footnotesize
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \column{0.38\textwidth}
    {\footnotesize\alert{VALE3: ganho do sentimento nunca é significativo ($p=0{,}194$, 880 \textit{runs}).}}\\[0.5ex]
    \begin{itemize}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{(n.s.; $p=0{,}194$, \textit{deep-dive} 880)} \\
        \midrule
    ...
    {\footnotesize IC \textit{bootstrap} do $\Delta$ médio (225 \textit{runs}) [$-$0{,}012; $+$0{,}018] --- cruza o zero; mesmo o teto $+$0{,}018 está muito abaixo de efeito útil. MDE 0{,}13--0{,}22 é \textit{por ativo} ($n$ pequeno/célula).}\end{center}
    {\scriptsize O nulo TEM IC; o 0{,}709 não tinha. Detalhes (MDE, \textit{forward-fill}, 0{,}609 vs 0{,}599) na reserva.}
  \end{columns}
\end{frame}
