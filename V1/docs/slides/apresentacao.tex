...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)}; a janela de teste calhou 69{,}5/30{,}5 --- e isso \alert{reforça} o alerta: acurácia 76{,}3\% mal supera 69{,}5\% (slide 10)
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill}: propaga passado, não vaza futuro de mercado. Risco residual é intradiário e, por plausibilidade (não auditado), tenderia a \alert{inflar} o sentimento
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  \begin{columns}[T]
    ...
    \column{0.46\textwidth}
    \begin{itemize}
      \item Transformer + FinBERT: \alert{ROC-AUC = 0{,}709}, acurácia 76{,}3\% \alert{($\approx$ classe majoritária 69{,}5\%)}
...
\begin{frame}{Três sinais de alerta}
  \begin{enumerate}
    ...
    \item \textbf{Acurácia suspeita:} 76{,}3\% supera a classe majoritária da janela de teste (69{,}5\%) por só $\sim$7 pontos
  ...
  \end{enumerate}
  \vspace{1ex}
  {\footnotesize Reamostragem 50/50 foi cogitada e descartada: quebraria a ordem temporal e a \textit{base rate} real. Compensei por \alert{peso na perda} (\texttt{pos\_weight}$\approx$1{,}38 / \texttt{scale\_pos\_weight}). O colapso persiste assim mesmo $\Rightarrow$ o desbalanceamento não é a causa raiz.}
  \vspace{1ex}
  \begin{block}{Decisão}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Cada experimento = 1 pergunta pré-registrada; métrica nunca escolhida \textit{a posteriori}. As execuções \alert{destroem} o positivo, não selecionam o melhor (oposto de \textit{p-hacking})
      \item Sem correção para múltiplas comparações: os testes-chave são \alert{não} significativos ($p=0{,}49$; $0{,}194$) --- correção só tornaria o nulo mais forte
    \end{itemize}
  \end{columns}
  \vspace{1ex}
  {\footnotesize Número central: \textbf{1.435} (8 experimentos). O $>$1.500 são diagnósticos exploratórios, não contados nos testes.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno:} 0{,}667 \textbf{não é lucrável} após custos --- não contradiz a EMH (forma semiforte fala de retorno ajustado a risco, não de AUC). \textit{Ensemble}+\textit{backtest} não gerou ganho (descontinuado)
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lccc}
        ...
        VALE3 & 0{,}599 & 0{,}635 & $+$0{,}036\,\alert{(n.s.)} \\
        \midrule
    ...
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3: ganho aparente, nunca significativo} --- 3 medidas distintas (slides 14/15/16, modelos/contrastes/$n$ diferentes); magnitude instável = a prova de que não é sinal. \textit{Deep-dive} 880 \textit{runs}: $p=0{,}194$.}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{($p=0{,}194$, n.s.)} \\
        \midrule
    ...
    \begin{itemize}
      ...
      \item \alert{Não é só falta de poder:} o mesmo desenho detecta o sinal de preço (0{,}667, estável) --- sensível a efeito real quando existe
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3: ganho aparente, nunca significativo} --- 3 medidas distintas (slides 14/15/16, modelos/contrastes/$n$ diferentes); magnitude instável = a prova de que não é sinal. \textit{Deep-dive} 880 \textit{runs}: $p=0{,}194$.}
  \end{columns}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill}: propaga passado (não vaza futuro de mercado); viés residual tenderia a \emph{inflar} o sinal de sentimento --- argumento de plausibilidade, não auditado
