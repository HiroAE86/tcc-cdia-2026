...
\begin{frame}{Dados: notícias + mercado}
  ...
  \end{columns}
  \vspace{0.5ex}
  {\footnotesize O colapso bimodal persiste no XGBoost (modelo da \textit{ablation}) $\Rightarrow$ desbalanceamento não é a causa raiz. Reamostrar 50/50 quebraria a ordem temporal e a \textit{base rate} (59/41); compensei por peso de classe (detalhe no Q\&A).}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textbf{Risco conhecido:} \textit{forward-fill} de sentimento propaga só o \alert{passado}; mesmo no pior caso (viés a favor do sentimento) o nulo $\Delta=+0{,}003$ seria limite \alert{superior} otimista. Risco intradiário (artigo pós-fechamento) não auditado --- detalhe no slide 19
  \end{itemize}
\end{frame}
