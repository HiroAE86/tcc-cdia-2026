...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \end{itemize}
    {\footnotesize Peso na perda dá peso equivalente à classe minoritária sem alterar a sequência nem a \textit{base rate}. O colapso \alert{persiste mesmo com o peso} $\Rightarrow$ desbalanceamento não era a causa raiz.}
  \end{columns}
\end{frame}
...
\begin{frame}{8 experimentos pré-registrados (1.435 execuções)}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize 1.435 pré-registrados $+$ $\sim$80 \textit{sanity checks} $\approx$ 1.515 total --- só os 8 pré-registrados têm p-valor.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade direcional, não retorno:} a autocorrelação de curto prazo é conhecida (Lo, 2004 --- mercados adaptativos) e $h=21$ com janelas sobrepostas a infla; o \textit{backtest} \textit{ensemble} \alert{não virou retorno após custos} e foi descontinuado
  \end{itemize}
  \vspace{0.5ex}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{(XGB, P vs P+S, 225; n.s.; $p=0{,}194$ --- s.15)} \\
        \midrule
