...
\begin{frame}{Dados: notícias + mercado}
  ...
  \end{columns}
  \vspace{0.5ex}
  {\footnotesize 50/50 quebraria a ordem temporal do \textit{walk-forward} e a \textit{base rate} real (59/41) --- o teste continua 59/41. Compensei por peso na perda/amostra, sem reamostrar. Decisivo: o colapso persiste no XGBoost (modelo da \textit{ablation}) compensado $\Rightarrow$ desbalanceamento não é a causa raiz (detalhe por família no Q\&A).}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. Os vários $\sim$0{,}66 do XGBoost (0{,}670 janela única; 0{,}662 só-preço; 0{,}665 preço$+$sent.; 0{,}667 CV) diferem só por protocolo/$n$ --- todos estáveis, o sentimento não move. A regra ``janela única $=$ artefato'' vale também para o XGBoost.}
\end{frame}
...
% ---------------------------------------------------------------- 11. Investigação
\begin{frame}{Bateria de investigação: 1.435 execuções pré-registradas}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{table}
      \footnotesize
    ...
    \begin{itemize}
      ...
      \item O espaço de busca de efeito positivo é 6 células ($3$ ativos $\times$ $2$ horizontes); Bonferroni só reforça o nulo
      \item Cada experimento = 1 pergunta e métrica pré-especificadas; nunca escolhidas \textit{a posteriori}
      \item \alert{Oposto de \textit{p-hacking}:} protocolo idêntico e pré-registrado \alert{destrói} o positivo, não seleciona o melhor
      \item $\approx$1.500 contando \textit{sanity checks}: os $\sim$80 \alert{não geram teste de hipótese} --- só os 8 centrais têm $p$
    \end{itemize}
  \end{columns}
  \vspace{1ex}
  {\footnotesize Reconciliação dos $\sim$80 \textit{sanity checks} no Q\&A.}
\end{frame}
