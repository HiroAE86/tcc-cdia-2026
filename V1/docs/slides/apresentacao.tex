...
\begin{frame}{Dados: notícias + mercado}
  ...
  \end{columns}
  \vspace{0.5ex}
  {\footnotesize A conclusão vem do XGBoost (\textit{ablation}) com \texttt{scale\_pos\_weight}; reamostrar 50/50 quebraria a ordem temporal e a \textit{base rate} real (59/41). O colapso persiste com peso $\Rightarrow$ desbalanceamento não é a causa raiz (slide 13).}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textbf{Risco conhecido:} \textit{forward-fill} de sentimento em dias sem notícia --- propaga só o \alert{passado} (preço e alvo vêm de OHLCV sem \textit{ff}); não fabrica efeito positivo (detalhado no slide 19)
    \item \textit{Features} técnicas (MM7/21, std21, \textit{lags}) são todas \alert{\textit{trailing}} (só janela passada); alvo $\text{Close}[t+h]$ estritamente futuro $\Rightarrow$ \textit{baseline} sem \textit{look-ahead}
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. \alert{O XGBoost 0{,}670 também é artefato de janela única} --- reaparece no \textit{ablation} como 0{,}665 (multi-seed); protocolos distintos, ambos não-estáveis.}
\end{frame}
...
% ---------------------------------------------------------------- 11. Investigação
\begin{frame}{Bateria de investigação: 8 experimentos, 1.435 execuções}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{table}
      \footnotesize
    ...
    \begin{itemize}
      ...
      \item \alert{Oposto de \textit{p-hacking}:} as execuções \alert{destroem} o positivo, não selecionam o melhor
    \end{itemize}
  \end{columns}
  \vspace{1ex}
  {\footnotesize 1.435 execuções pré-registradas (pergunta e métrica a priori, têm p-valor) $+$ $\sim$80 \textit{sanity checks} $\approx$ 1.515 --- nenhum \textit{sanity check} gera teste de hipótese.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno} (Lo, 2004; $h=21$ sobreposto infla a autocorrelação)
  \end{itemize}
  \vspace{0.5ex}
  {\footnotesize Captura momentum/autocorrelação de curto prazo (Lo, 2004); $h=21$ com janelas sobrepostas infla a autocorrelação do alvo. \alert{0{,}658 $=$ 1 janela (IC largo); 0{,}667 $=$ média 5 \textit{folds} CV. \textbf{O preço vence o sentimento nos dois.}}}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lccc}
        ...
        VALE3 & 0{,}599 & 0{,}635 & {\footnotesize $+$0{,}036 (n.s.)} \\
        \midrule
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item Era o único ativo com ganho aparente ($+$0{,}036\,\alert{(n.s.; exp.\ distinto)}) na análise principal --- e a única das $3\times2=6$ células com ganho: sob múltiplas comparações, a esperada por acaso
      ...
      \item \alert{\textbf{Nunca significativo}} ($p=0{,}194$): o sinal troca de direção entre experimentos (\textit{magnitudes} $+$0{,}036, $+$0{,}058, aqui negativo) --- a própria evidência de que \alert{não há sinal real}
    \end{itemize}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & {\footnotesize $+$0{,}058 (n.s.)} \\
        \midrule
        \textbf{Média} & \textbf{0{,}662} & 0{,}480 & 0{,}665 & \alert{\textbf{$+$0{,}003}} \\
        \bottomrule
      \end{tabular}}
    \end{table}
    {\scriptsize Dois de três ativos \alert{pioram} com sentimento (ITUB4 $-$0{,}033; PETR4 $-$0{,}016). O 0{,}662 é a média PRICE-only do \textit{ablation} (XGBoost, 225 \textit{runs}); difere do 0{,}667 do CV por desenho --- todos estáveis $\sim$0{,}66, o sentimento não move.}
    \begin{center}\Large\alert{$\Delta = +0{,}003$ \ ($p = 0{,}49$)} --- estatisticamente zero\end{center}
    {\scriptsize O \textit{n.s.} da célula VALE3 é confirmado pelo \textit{deep-dive} dedicado (880 \textit{runs}, $p=0{,}194$); o teste formal do sentimento é o $\Delta$ agregado ($p=0{,}49$) --- células isoladas não recebem teste próprio (múltiplas comparações). Reconciliação MDE \textit{vs} IC agregado: reserva.}
  \end{columns}
\end{frame}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{ff} toca só o sentimento; no pior caso favorece-o, e ainda assim $\Delta=+0{,}003$ --- não fabrica efeito (detalhe no Q\&A)
      \item Instabilidade arquitetural do Transformer
      \item \textit{Backtest ensemble} descontinuado: sem ganho líquido após custos/\textit{slippage} (negativo reportado; não reivindico lucratividade)
    \end{itemize}
    \column{0.48\textwidth}
...
\begin{frame}{Reserva --- MDE \textit{vs} IC do $\Delta$ agregado}
  ...
  \vspace{1ex}
  {\footnotesize O IC [$-$0{,}012; $+$0{,}018] é \alert{empírico} --- vem do \textit{bootstrap} dos 225 \textit{runs}, não de uma suposição de independência. Mesmo o teto $+$0{,}018 está $\sim$3$\times$ abaixo do MDE de efeito útil. (Reserva: estrutura de correlação dos \textit{folds} / $n$ efetivo.) O mesmo desenho \alert{detecta preço} (0{,}667): nulo conclusivo no agregado, não por ativo.}
\end{frame}
