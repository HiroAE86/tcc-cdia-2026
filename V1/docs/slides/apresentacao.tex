...
\begin{frame}{Pergunta de pesquisa}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{(b) Robustez metodológica}
      ...
      sementes e testes formais? \alert{Não:} $\Delta$ sentimento
      $= +0{,}003$ ($p=0{,}49$); o 0{,}709 é artefato de janela única.
    \end{block}
  \end{columns}
  \vspace{1.5ex}
  \centering
...
\begin{frame}{Dados: notícias + mercado}
  ...
  \end{columns}
  \vspace{0.5ex}
  {\footnotesize Não reamostrei 50/50: quebraria a ordem temporal do \textit{walk-forward} e a \textit{base rate} real (59/41). Compensei por família: XGBoost \texttt{scale\_pos\_weight}; \textit{sklearn} \texttt{class\_weight}; neurais com \texttt{BCEWithLogitsLoss} \texttt{pos\_weight}$\approx$1{,}38; TCN (\texttt{Sigmoid}+\texttt{BCELoss}) por peso de amostra/\textit{threshold}. Decisivo: o colapso persiste no XGBoost (modelo da \textit{ablation}), compensado $\Rightarrow$ desbalanceamento não é a causa raiz.}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textbf{Risco conhecido:} \textit{forward-fill} de sentimento propaga só o \alert{passado}. O viés intradiário não auditado (artigo pós-fechamento atribuído ao dia $t$) só poderia \alert{inflar} o sentimento; como mesmo inflado $\Delta=+0{,}003$ ($p=0{,}49$), corrigi-lo \alert{só reforça o nulo} --- nunca o reverte para positivo (slide 19)
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. XGBoost 0{,}670 $=$ janela única (sentimento bruto \textit{vs embeddings}); o valor multi-seed está no slide 16.}
\end{frame}
...
% ---------------------------------------------------------------- 11. Investigação
\begin{frame}{Bateria de investigação: 1.435 execuções pré-registradas ($\approx$1.500 com \textit{sanity checks})}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{table}
      \footnotesize
    ...
    \begin{itemize}
      ...
      \item Cada experimento = 1 pergunta e métrica pré-especificadas; nunca escolhidas \textit{a posteriori}
      \item \alert{Oposto de \textit{p-hacking}:} protocolo idêntico e pré-registrado \alert{destrói} o positivo, não seleciona o melhor (Bonferroni / espaço de busca no Q\&A)
    \end{itemize}
  \end{columns}
  \vspace{1ex}
  {\footnotesize Só os 8 experimentos centrais geram teste de hipótese (reconciliação dos $\sim$80 \textit{sanity checks} no Q\&A).}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item O IC de janela única é tão largo que engloba o 0{,}709 --- a separação do \textit{baseline} só aparece no CV (slide 14)
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lccc}
        ...
        VALE3 & 0{,}599 & 0{,}635 & {\footnotesize $+$0{,}036; $p=0{,}194$ (n.s., s.15)} \\
        \midrule
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & {\footnotesize $+$0{,}058; $p=0{,}194$ (n.s., s.15)} \\
        \midrule
    ...
    {\scriptsize IC estreito por agregação (225 \textit{runs}); o MDE 0{,}13--0{,}22 é \alert{por ativo} ($n$ pequeno) --- não há contradição. Todos os \textit{baselines} de preço $\sim$0{,}66 (variação é protocolo, não sinal). Espaço de busca de efeito positivo $=$ 6 células ($3\times2$); Bonferroni só reforça o nulo.}
  \end{columns}
\end{frame}
