...
\begin{frame}{Contexto e motivação}
  ...
  \vspace{1ex}
  \begin{block}{Lacuna}
    Evidência empírica ainda limitada para o mercado brasileiro, sobretudo
    com avaliação multi-janela e testes de hipótese formais.
  \end{block}
\end{frame}
...
\begin{frame}{Bateria de investigação: 1.435 execuções --- pergunta e métrica definidas a priori}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \alert{Oposto de \textit{p-hacking}:} protocolo idêntico, fixo a priori, \alert{testa} o positivo, não seleciona o melhor
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno} (Lo, 2004; $h=21$ sobreposto infla a autocorrelação). \textbf{Não reivindico lucratividade:} é momentum/autocorrelação; o \textit{backtest} exploratório foi \textbf{negativo} (Sharpe $-$1{,}29) --- reforça o nulo, mas fica fora da contribuição por fragilidade (s.19/reserva)
  \end{itemize}
  \vspace{0.5ex}
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Resultado: \alert{não significativo} (Wilcoxon $p = 0{,}194$) --- a exceção não sobrevive ao escrutínio
...
\begin{frame}{Contribuições e protocolos mínimos propostos}
  \begin{columns}[T]
    ...
    \begin{enumerate}
      ...
      \item Demonstração quantificada do viés de janela única em dados brasileiros (1.435 execuções)
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Backtest ensemble} exploratório e \textbf{negativo} (Sharpe $-$1{,}29 \textit{vs} \textit{buy-and-hold} 3{,}25 a 10 \textit{bps}): reforça o nulo, mas fica fora da contribuição principal por fragilidade metodológica (2/20 \textit{seeds} sobreviventes) --- não reivindico lucratividade
    \end{itemize}
    \column{0.48\textwidth}
