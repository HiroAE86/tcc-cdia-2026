...
\begin{frame}{Três sinais de alerta}
  \begin{enumerate}
    ...
    \item \textbf{Acurácia suspeita:} 76{,}3\% supera por só $\sim$7 pontos a classe majoritária \emph{da janela de teste} (69{,}5\% ``Sobe'', 123/177)\\{\footnotesize Desbalanceamento difere por período: treino 59/41, janela de teste 69{,}5/30{,}5 (\textit{split} cronológico).}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize VALE3: ganho aparente, nunca significativo (\textit{deep-dive} 880 \textit{runs}, $p=0{,}194$).}
  \end{columns}
\end{frame}
