...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \alert{Resíduo intradiário não auditado --- conclusão nula é robusta à direção do viés: se o \textit{ff} favorece o sentimento, $+$0{,}003 já é teto otimista (slide 19)}
  \end{itemize}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \alert{\textbf{1.435 execuções pré-registradas = 8 experimentos, 8 p-valores.}} Os $\sim$65 \textit{sanity checks} restantes (até $>$1.500) NÃO geram teste de hipótese. Só os 8 pré-registrados têm p-valor.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \end{itemize}
  \vspace{0.5ex}
  {\footnotesize AUC mede separabilidade, não lucro; captura momentum/autocorrelação de curto prazo (Lo, 2004). $h=21$ com janelas sobrepostas infla a autocorrelação do alvo; o \textit{backtest} \textit{ensemble} (slide 11) não converteu a separabilidade em retorno após custos --- compatível com eficiência fraca, não semi-forte.}
\end{frame}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    {\scriptsize Mesmo desenho detecta preço (0{,}667 estável); sentimento 44--74$\times$ abaixo do MDE.\\
    Preço VALE3 = 0{,}609 (ablation) vs 0{,}599 (CV, s.14): protocolos/$n$ distintos; invariante é o $p$, não o nível.}
  \end{columns}
\end{frame}
