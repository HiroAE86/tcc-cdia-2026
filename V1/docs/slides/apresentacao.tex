...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- compensado por peso de classe (XGB \texttt{scale\_pos\_weight}; \texttt{pos\_weight}$\approx$1{,}38 no \texttt{BCEWithLogits}), \alert{não por reamostragem}
    \end{itemize}
    {\scriptsize \alert{50/50 (sugestão do orientador) avaliada e preterida:} reamostrar quebra a ordem temporal e a \textit{base rate} real (59/41). O colapso bimodal persiste mesmo com peso de classe $\Rightarrow$ desbalanceamento não era a causa raiz.}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \alert{Resíduo intradiário não auditado --- conclusão nula é robusta à direção do viés: SE o \textit{ff} favorece o sentimento, $+$0{,}003 é limite superior; SE for contra, o efeito real é ainda menor --- nos dois casos o nulo se mantém (slide 19)}
  \end{itemize}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. (XGBoost 0{,}670 é \textit{janela única}; cai a 0{,}665 \textit{multi-seed} na ablation, s.16.)}
\end{frame}
...
\begin{frame}{8 experimentos pré-registrados (1.435 execuções)}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Só os 8 pré-registrados têm p-valor; os $\sim$65 \textit{sanity checks} restantes (até $>$1.500) NÃO geram teste de hipótese.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \end{itemize}
  \vspace{0.5ex}
  {\footnotesize Captura momentum/autocorrelação de curto prazo (Lo, 2004); $h=21$ com janelas sobrepostas infla a autocorrelação do alvo.}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{(n.s.; $p=0{,}194$, 880 \textit{runs})} \\
        \midrule
    ...
    \begin{center}\Large\alert{$\Delta = +0{,}003$}\ \ ($p = 0{,}49$)\\[0.3ex]
    {\footnotesize IC \textit{bootstrap} [$-$0{,}012; $+$0{,}018] --- cruza o zero; MDE absoluto 0{,}13--0{,}22}\end{center}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill} (só sentimento; intradiário, não auditado) --- conclusão nula é robusta à direção do viés: se favorece o sentimento, $\Delta=+0{,}003$ é teto otimista; se for contra, o efeito real é ainda menor
