...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill}: propaga o passado, não vaza retorno futuro de mercado; resíduo é só intradiário (notícia pós-17h no dia $t$), \alert{não auditado}. Como a conclusão é \alert{nula}, qualquer viés que favoreça o sentimento só torna o nulo mais conservador
  \end{itemize}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções pré-registradas (8 experimentos) com p-valor; os $>$65 \textit{sanity checks} até $\sim$1.500 não geram teste de hipótese. As $3\times2=6$ células ativo$\times$horizonte do \textit{ablation} são o único espaço de busca de efeito positivo: 1 deu ganho --- o esperado por acaso. Os contrastes com direção (ITUB4/PETR4: preço vence) sobrevivem a Bonferroni das 6 células.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item A estimativa de janela única é tão incerta (IC largo) que \alert{não distingue} 0{,}709 do \textit{baseline} --- nesta janela nem o \textit{baseline} se separa do acaso; a única conclusão honesta de janela única é que ela \alert{não resolve nada}. A separação só aparece no multi-seed/CV (slides 13--16), onde a direção inverte
    \item \alert{AUC mede separabilidade, não lucro} --- não reivindico lucratividade. O 0{,}658 (aqui, janela única) vem de momentum de curto prazo; não contradiz a HME --- separabilidade $\neq$ retorno ajustado a risco após custos (detalhe e \textit{backtest} exploratório na reserva de Q\&A)
  \end{itemize}
\end{frame}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \end{table}
    \begin{itemize}
      \item Média XGB dos 3 ativos $=$ \textit{baseline} autoregressivo CV $=$ 0{,}667 (mesma medida, não coincidência; cf.\ 0{,}658 da janela única, slide 12)
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item IC 95\% $[-0{,}012;\ +0{,}018]$ cruza o zero --- \emph{não detectável neste $n$} (MDE 0{,}13--0{,}22; sentimento 44--74$\times$ abaixo), não ``efeito zero''
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill}: propaga passado (não vaza futuro de mercado). Resíduo é apenas intradiário (notícia pós-17h atribuída ao dia $t$), escopo limitado e \alert{não auditado}. O \textit{ff} só afeta as \textit{features} de sentimento (o preço vem de OHLCV diário sem \textit{ff}); qualquer viés inflaria o sentimento, nunca o \textit{baseline} --- o $\Delta=+0{,}003$ é teto otimista do sentimento
