...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill}: propaga o passado, não vaza futuro de mercado (risco residual é intradiário --- detalhado nas limitações)
  \end{itemize}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno:} esse \textit{baseline} de preço ($\sim$0{,}66 em qualquer recorte) não contradiz a EMH (forma semiforte fala de retorno ajustado a risco, não de AUC). \textit{Ensemble}+\textit{backtest} exploratório não converteu o 0{,}66 em retorno e foi descontinuado --- não reivindico estudo formal de lucratividade
  \end{itemize}
\end{frame}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3: o ganho do sentimento nunca é estatisticamente significativo} (\textit{deep-dive} 880 \textit{runs}, $p=0{,}194$). Ganho aparente aqui $+$0{,}036; medidas distintas nos slides 14/15/16 --- ver slide 15.}
  \end{columns}
\end{frame}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3: o ganho do sentimento nunca é estatisticamente significativo.} O $+$0{,}058 desta ablation é isolado e não tem teste por célula; a significância formal vem do \textit{deep-dive} de 880 \textit{runs} ($p=0{,}194$). Três experimentos distintos (slides 14/15/16) $\Rightarrow$ magnitudes diferentes; instabilidade = prova de que não é sinal.}
  \end{columns}
\end{frame}
