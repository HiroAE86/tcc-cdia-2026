...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill} propaga o passado (não vaza retorno futuro); resíduo só intradiário, não auditado
    \item \alert{O \textit{ff} afeta só o sentimento; preço e alvo vêm de OHLCV diário sem \textit{ff}}
  \end{itemize}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções pré-registradas (8 experimentos); os $>$65 \textit{sanity checks} não geram p-valor. (Bonferroni e as 6 células: reserva de Q\&A.)}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não lucro} --- não reivindico lucratividade (HME, momentum e \textit{backtest} exploratório na reserva de Q\&A)
  \end{itemize}
\end{frame}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    \end{table}
    \begin{itemize}
      \item Média XGB dos 3 ativos $=$ \textit{baseline} autoregressivo CV $=$ 0{,}667. O 0{,}658 (1 janela, IC largo) e o 0{,}667 (média 5 \textit{folds}) não comparam como ``melhora'' --- a diferença cabe no IC; o que muda entre janela e CV é a \alert{estabilidade}, não o nível
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  ...
  \end{columns}
  \vspace{0.5ex}
  {\scriptsize O 0{,}667 da célula VALE3/P+S é coincidência numérica com a média CV do slide 14 --- medidas distintas (1 célula de \textit{ablation} vs média de 3 ativos).}
\end{frame}
