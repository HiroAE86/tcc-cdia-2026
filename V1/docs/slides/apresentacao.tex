...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill}: propaga passado, não vaza futuro de mercado. Risco residual é intradiário e \alert{robusto à direção do viés}: se favorece o sentimento, o nulo é limite superior otimista; se for contra, o efeito real é ainda menor (não auditado)
  \end{itemize}
\end{frame}
...
\begin{frame}{Três sinais de alerta}
  ...
  \end{enumerate}
  \vspace{1ex}
  {\footnotesize Reamostragem 50/50 foi cogitada e descartada: quebraria a ordem temporal e a \textit{base rate} real. Compensei por \alert{peso na perda onde aplicável} (\texttt{pos\_weight}$\approx$1{,}38 nos modelos com \textit{logits}; \texttt{scale\_pos\_weight} no XGBoost; peso de amostra/\textit{threshold} no TCN/Sigmoid, que usa \texttt{BCELoss}). O colapso persiste assim mesmo $\Rightarrow$ o desbalanceamento não é a causa raiz.}
  \vspace{1ex}
  \begin{block}{Decisão}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Número central: \textbf{1.435} (8 experimentos). O $>$1.500 são diagnósticos exploratórios (\textit{sanity checks}, ajuste de visualização) --- \alert{nenhum gera teste de hipótese}; só os 8 pré-registrados têm $p$-valor.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno:} esse \textit{baseline} de preço ($\sim$0{,}66 em qualquer recorte) \textbf{não é lucrável} após custos --- não contradiz a EMH (forma semiforte fala de retorno ajustado a risco, não de AUC). \textit{Ensemble}+\textit{backtest} não gerou ganho (descontinuado)
  \end{itemize}
\end{frame}
...
\begin{frame}{E a exceção VALE3? \textit{Deep-dive} com 880 execuções}
  \begin{columns}[T]
    ...
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3: ganho instável, nunca significativo} ($p=0{,}194$ no \textit{deep-dive} de 880 \textit{runs}).}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667 & $+$0{,}058\,\alert{(n.s.)} \\
        \midrule
    ...
    \begin{itemize}
      ...
      \item \alert{Não é só falta de poder:} o mesmo desenho detecta o sinal de preço (PRICE médio 0{,}662, estável) --- sensível a efeito real quando existe
    \end{itemize}
    \vspace{0.5ex}
    {\footnotesize \alert{VALE3: ganho instável, nunca significativo} ($p=0{,}194$ no \textit{deep-dive} de 880 \textit{runs}).}
  \end{columns}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill}: propaga passado (não vaza futuro de mercado); conclusão nula é robusta a ambas as direções do viés (a favor $\Rightarrow$ nulo é limite otimista; contra $\Rightarrow$ efeito ainda menor) --- não auditado
