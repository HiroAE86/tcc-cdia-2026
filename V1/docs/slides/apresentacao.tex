...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill} propaga o passado (não vaza retorno futuro); resíduo só intradiário, não auditado. O \textit{ff} afeta só o sentimento; preço e alvo vêm de OHLCV sem \textit{ff}
    \item \alert{Conclusão nula é robusta à direção do viés: se o \textit{ff} favorece o sentimento, $+$0{,}003 já é limite superior otimista}
  \end{itemize}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções pré-registradas (8 experimentos); os $>$65 \textit{sanity checks} não geram p-valor. (Múltiplas comparações / Bonferroni e os dois \textit{claims}: reserva de Q\&A.)}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item A estimativa de janela única é tão incerta (IC largo) que \alert{não distingue} 0{,}709 do \textit{baseline} --- a separação do acaso só emerge no CV/multi-seed (slide 14), onde a direção inverte
    \item \alert{0{,}658 (1 janela, IC largo) e 0{,}667 (média CV) são a MESMA grandeza sob protocolos distintos} --- não comparam como ``melhora''
    \item \alert{AUC mede separabilidade, não lucro} --- não contradiz a HME: momentum/autocorrelação de curto prazo (h$=$21 sobreposto infla), e \textit{backtest} exploratório não virou retorno após custos
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{table}
      ...
      \begin{tabular}{lcccc}
        ...
        VALE3 & 0{,}609 & 0{,}510 & 0{,}667$^\dagger$ & $+$0{,}058\,\alert{(n.s.; exp.\ distinto)} \\
        \midrule
        \textbf{Média} & \textbf{0{,}662} & 0{,}480 & 0{,}665 & \alert{\textbf{$+$0{,}003}} \\
        \bottomrule
      \end{tabular}}
    \end{table}
    {\scriptsize $^\dagger$ coincidência numérica com a média CV do slide 14 --- \alert{medida distinta} (1 célula vs média de 3 ativos).}\\[0.3ex]
    \begin{center}\large\alert{$\Delta = +0{,}003$}\ \ ($p = 0{,}49$)\end{center}
    \begin{itemize}
      \item IC 95\% cruza o zero --- \emph{não detectável neste $n$}, não ``efeito zero'' (MDE 0{,}13--0{,}22 na reserva)
    \end{itemize}
  \end{columns}
\end{frame}
