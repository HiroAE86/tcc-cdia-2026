...
\begin{frame}{Reserva --- Por que AUC? E o \textit{backtest}}
  \begin{block}{Por que AUC é a métrica}
    {\footnotesize AUC mede \textbf{separabilidade direcional} --- exatamente a pergunta --- e era a métrica do 0{,}709 que eu auditava (consistência). O nulo é robusto: o mesmo desenho move a AUC no \textbf{preço} (0{,}667) e \alert{não} no sentimento. \textbf{Honestidade:} a ablação decisiva mede só $\Delta$AUC; calibração (Brier/ECE) foi computada na fase diagnóstica, não nos 225 \textit{runs}. Claim restrito: ``nenhum \textbf{ganho discriminativo} detectável''.}
  \end{block}
