...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item {\footnotesize (a janela de teste cronológica calha em 69{,}5\% ``Sobe'' --- períodos distintos)}
    \end{itemize}
  \end{columns}
  \vspace{0.5ex}
  {\footnotesize Não reamostrei 50/50 (quebraria ordem temporal $+$ \textit{base rate}); compensei por peso (\texttt{scale\_pos\_weight} no XGBoost da \textit{ablation}, \texttt{class\_weight} nos \texttt{sklearn}, peso na perda nos neurais). O colapso bimodal persiste mesmo assim --- desbalanceamento não é a causa raiz (slide 13).}
\end{frame}
...
\begin{frame}{O resultado que ``confirmou'' a hipótese}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize Estimativa pontual, 1 janela, 1 semente, sem IC --- refutado nos slides 13--16. \alert{O XGBoost 0{,}670 também é artefato de janela única.}}
\end{frame}
...
% ---------------------------------------------------------------- 11. Investigação
\begin{frame}{8 experimentos com pergunta e métrica definidas a priori (1.435 execuções)}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{table}
      \footnotesize
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade direcional, não retorno:} a autocorrelação de curto prazo é conhecida (Lo, 2004 --- mercados adaptativos) e $h=21$ com janelas sobrepostas a infla; o \textit{backtest} \textit{ensemble} foi descontinuado \alert{após não evidenciar ganho líquido de trading após custos/slippage} --- resultado negativo reportado, não omitido; não reivindico lucratividade
  \end{itemize}
  \vspace{0.5ex}
...
\begin{frame}{Expanding-window CV: a inversão}
  \begin{columns}[T]
    ...
    Na VALE3 as faixas se cruzam e o ganho \textbf{não é significativo} ($p=0{,}194$).}
    \column{0.42\textwidth}
    \begin{block}{Invariante (liderar por aqui)}
      \alert{Na VALE3 o ganho do sentimento \textbf{nunca} é significativo ($p=0{,}194$, 880 \textit{runs}).}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    {\scriptsize Reconciliação MDE \textit{vs} IC agregado: slide de reserva.}
  \end{columns}
\end{frame}
...
\begin{frame}{Limitações e trabalhos futuros}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textit{Look-ahead} residual do \textit{forward-fill} (intradiário, não auditado): o \textit{ff} toca \alert{só o sentimento} --- preço e alvo vêm de OHLCV sem \textit{ff}. Argumento estrutural: o canal de vazamento plausível (notícia pós-fechamento $\sim$17h) atua \alert{a favor} do sentimento --- não auditado, mas mesmo no pior caso o ganho observado é $+$0{,}003. O \textit{ff} \alert{não FABRICA um efeito positivo inexistente}
...
\begin{frame}{Reserva --- VALE3 nos três experimentos}
  ...
  \begin{table}
    ...
    \begin{tabular}{llccl}
      \toprule
      \textbf{Slide} & \textbf{Experimento} & \textbf{$n$} & \textbf{Modelo} & $\boldsymbol{\Delta}$ \\
      \midrule
      14 & CV \textit{expanding} (Transf.\ vs preço) & 5$\times$5 & Transformer & $+$0{,}036 \\
      16 & Ablation (preço vs preço$+$sent.)        & 225        & XGBoost     & $+$0{,}058 \\
  ...
  O invariante é o \textbf{$p$}, não o nível. Sinal instável = a própria evidência de que não é sinal real.}
\end{frame}
