...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- tratado por peso na perda (\texttt{pos\_weight}$\approx$1{,}38 / \texttt{scale\_pos\_weight}), não por reamostragem (preserva ordem temporal e \textit{base rate})
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline}
  ...
  \begin{itemize}
    ...
    \item \textit{Forward-fill}: propaga o passado, não vaza retorno futuro de mercado; resíduo intradiário tratado nas limitações. A direção do viés favorece o sentimento $\Rightarrow$ a conclusão nula é \alert{conservadora} (limite superior)
  \end{itemize}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item O próprio 0{,}709 já era o \alert{máximo} selecionado de 8 células (slide 9) --- outra face do mesmo viés; as 1.435 execuções testam hipótese pré-registrada, não o melhor
    \end{itemize}
  \end{columns}
  \vspace{1ex}
...
\begin{frame}{Ablation: o sentimento adiciona algo ao preço?}
  \begin{columns}[T]
    ...
    \end{table}
    \begin{itemize}
      \item \alert{Veredito: $\Delta = +0{,}003$} (Wilcoxon $p = 0{,}49$; IC 95\% $[-0{,}012;\ +0{,}018]$) --- SENT sozinho \alert{abaixo do acaso}
      \item Conclusão: ``nenhum efeito \emph{detectável} neste $n$'' (exclui efeitos $\geq$0{,}13 em AUC; menores ficam indistinguíveis de zero --- não é ``efeito zero''). Aumentar $n$ é direção futura
    \end{itemize}
    \vspace{0.5ex}
