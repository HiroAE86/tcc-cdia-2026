...
\begin{frame}{Dados: notícias + mercado}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Desbalanceamento: 59/41 ``Sobe''/``Desce'' \emph{(treino)} --- compensado por \alert{peso na perda}, \alert{não por reamostragem} --- preserva ordem temporal e \textit{base rate}
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Investigação metodológica: 8 experimentos, 1.435 execuções}
  ...
  \end{columns}
  \vspace{1ex}
  {\footnotesize \textbf{1.435} execuções nos 8 experimentos pré-registrados (cada um com $p$-valor); os demais até $\sim$1.500 são \textit{sanity checks} sem teste de hipótese.}
\end{frame}
...
\begin{frame}{Primeiro confronto: hierarquia de \textit{baselines}}
  ...
  \begin{itemize}
    ...
    \item \alert{AUC mede separabilidade, não retorno} --- e não reivindico nada sobre lucratividade (fora do escopo). O 0{,}667 vem de momentum/autocorrelação de curto prazo (alvo $h=21$ com janelas sobrepostas infla autocorrelação); não contradiz a HME semiforte, que fala de retorno ajustado a risco após custos. O \textit{backtest} exploratório não deu sinal e foi descontinuado justamente por isso
  \end{itemize}
\end{frame}
